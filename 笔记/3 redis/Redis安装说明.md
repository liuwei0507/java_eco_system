...
注意：由于默认安装Redis容器内部没有/etc/redis/redis.conf文件，所以预先要在Linux上创建好文件，再进行挂载

```shell
mkdir -p /Users/weiliu/Documents/software/docker_data/redis/config
```

```shell
touch /Users/weiliu/Documents/software/docker_data/redis/redis.conf
```

```shell
docker run -p 6379:6379 --name redis \
-v /Users/weiliu/Documents/software/docker_data/redis/data:/data \
-v /Users/weiliu/Documents/software/docker_data/redis/config/redis.conf:/etc/redis/redis.conf \
-d redis:6.2.6 redis-server /etc/redis/redis.conf
```

## 2.3 配置Redis数据持久化

因为默认Redis容器的数据没有持久化到硬盘（只是在内存中），重启Redis之后，数据会丢失

因此在 /Users/weiliu/Documents/software/docker_data/redis/config/redis.conf中加入
